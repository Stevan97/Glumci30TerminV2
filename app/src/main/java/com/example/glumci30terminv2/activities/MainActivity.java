...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Glumac> glumacList = null;
    private ArrayAdapter<Glumac> glumacArrayAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences = getSharedPreferences("Glumci.FirstLaunch", MODE_PRIVATE);

    }

    /**
     * Prikazivanje liste Glumaca
     */
    private void prikaziListuGlumca() {
        listViewMain = findViewById(R.id.list_view_MAIN);
        try {
            glumacList = getDatabaseHelper().getGlumac().queryForAll();
            glumacArrayAdapter = new ArrayAdapter<>(this, R.layout.list_array_adapter, R.id.list_array_text_view, glumacList);
            listViewMain.setAdapter(glumacArrayAdapter);
            listViewMain.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

                    glumac = (Glumac) listViewMain.getItemAtPosition(position);

                    Intent intent = new Intent(MainActivity.this, DetailActivity.class);
                    intent.putExtra("id", glumac.getId());
    ...
    private void addGlumac() {
        ...
        final EditText editNaziv = dialog.findViewById(R.id.add_glumac_naziv);
        final EditText editBiografija = dialog.findViewById(R.id.add_glumac_biografija);
        final EditText editOcena = dialog.findViewById(R.id.add_glumac_ocena);
        final EditText editDatum = dialog.findViewById(R.id.add_glumac_datum_rodjenja);
        ...
        Button confirm = dialog.findViewById(R.id.add_glumac_button_confirm);
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String biografija = editBiografija.getText().toString();
                ...
                glumac.setDatumRodjenja(datum);

                try {
                    getDatabaseHelper().getGlumac().create(glumac);
                    dialog.dismiss();
                    refresh();

                    message1 = new SpannableString("Uspesno kreiran Glumac:  ");
        ...
        Button cancel = dialog.findViewById(R.id.add_glumac_button_cancel);
    ...
    /**
     * Osvezavanje prve Aktivnosti
     */
    private void refresh() {
        listViewMain = findViewById(R.id.list_view_MAIN);
        if (listViewMain != null) {
            glumacArrayAdapter = (ArrayAdapter<Glumac>) listViewMain.getAdapter();
            if (glumacArrayAdapter != null) {
                try {
                    glumacList = getDatabaseHelper().getGlumac().queryForAll();
                    glumacArrayAdapter.clear();
                    glumacArrayAdapter.addAll(glumacList);
                    glumacArrayAdapter.notifyDataSetChanged();
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
